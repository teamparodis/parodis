function uispider_plot(uiax, P, varargin)
%spider_plot Create a spider or radar plot with individual axes.
%
% Syntax:
%   spider_plot(uiax,P)
%   spider_plot(uiax,P, Name, Value, ...)
%
% Input Arguments:
%   (Required)
...
% Author:
%   Matthias Hoffmann
%   2021-01-21: Changes to all plot calls, added uiax object and removed some uistack calls, to be 
%   able to use spider_plot in uifigures.
%
...
for ii = 1:num_data_groups
    ...
    % Plot data points
    plot(uiax,x_circular, y_circular,...
...
if ~strcmp(axes_labels, 'none')
    ...
    for ii = 1:length(axes_labels)
        ...
        % Display text label
        text(uiax,x_axes(ii)+x_pos, y_axes(ii)+y_pos, axes_labels{ii},...
            'Units', 'Data',...
    ...
    for ii = 1:theta_end_index
        ...
        for jj = 2:length(rho)
            ...
            text(uiax,x_axes(jj), y_axes(jj), text_str,...
                'Units', 'Data',...
                'Color', 'k',...
