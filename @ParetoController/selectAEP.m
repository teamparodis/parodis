function [idx, utility] = selectAEP(varargin)
...
EP = zeros(dim);
[~,EP_pos] = min(normedFront);

for i = 1:dim
    EP(i,:) = normedFront(EP_pos(i),:);
end

utilityAEP = NaN(size(normedFront,1),1);

for j=ParetoController.paretoSetDiff(1:size(normedFront,1),EP_pos)  % skip extreme points
    utilityAEP(j,1) = coneAngle(normedFront(j,:), EP);
end

utilityUP = coneAngle(zeros(1,dim),EP);
utility = abs(utilityAEP-utilityUP);
[~, idx] = min(utility);
